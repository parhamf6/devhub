...
export default function NavbarV2() {
  ...
  return (
      ...
      <nav className="flex items-center justify-between px-4 py-3 max-w-7xl mx-auto">
        ...
        <div className="hidden md:flex gap-4 items-center">
          ...
          <div className="relative group">
            ...
            <div
              className={`absolute top-full bg-background bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-20 shadow-lg rounded-lg py-2 w-48 z-50 border transition-all duration-100 ${moreOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'
                }`}
              onMouseEnter={() => setMoreOpen(true)}
              onMouseLeave={() => setMoreOpen(false)}
            >
              ...
              <div className="flex flex-col h-full">
                  ...
                  <div className="space-y-4">
                    {navLinks.map((link, index) => (
                      <motion.div key={link.href} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }}>
                        <Link href={link.href} className="flex items-center gap-3 text-foreground text-lg font-medium py-3 px-4 rounded-lg hover:bg-accent/50 transition-colors duration-200" onClick={() => setOpen(false)}>
                    ...
                    <div className="pt-6 border-t">
                      ...
                      <div className="space-y-2">
                        {moreLinks.map((link, index) => (
                          <motion.div key={link.href} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: (navLinks.length + index) * 0.1 }}>
                            <Link href={link.href} className="flex items-center gap-3 text-foreground text-base py-2 px-4 rounded-lg hover:bg-accent/50 transition-colors duration-200" onClick={() => setOpen(false)}>
                              <link.icon className="w-4 h-4" />
                              {link.name}
                            </Link>
                          </motion.div>
                ...
                <div className="p-6 border-t">
                  <div className="flex items-center justify-center gap-4">
                    <Link href="/login" className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-foreground hover:bg-accent rounded-lg transition-colors duration-200" onClick={() => setOpen(false)}>
                      <LogIn className="w-4 h-4" />
                      Login
                    </Link>
                    <Link href="/signup" className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors duration-200" onClick={() => setOpen(false)}>
                      <UserPlus className="w-4 h-4" />
                      Sign Up
                    </Link>
                  </div>
    ...
    <div className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-background border-t border-border shadow-md">
    <div className="flex justify-between items-center px-4 py-2">
        ...
        {/* More Dropdown (Mobile too) */}
        <DropdownMenu>
        <DropdownMenuTrigger asChild>
            <button className="flex flex-col items-center text-xs text-muted-foreground hover:text-primary transition-colors">
