...
import jsonYamlConverter, { converterInstance, ConversionOptions, ConversionResult } from '@/lib/tools/tool/jsonyaml';
...
const defaultOptions: ConversionOptions = {
    ...
    // Swap direction
    const handleSwap = () => {
        const newMode = options.mode === 'json-to-yaml' ? 'yaml-to-json' : 'json-to-yaml';
    ...
    const handleDownload = () => {
        if (!output) return;
        const format = options.mode === 'json-to-yaml' ? 'yaml' : 'json';
    ...
    // Insert example
    const handleExample = () => {
        const example = options.mode === 'json-to-yaml' ? EXAMPLES.json : EXAMPLES.yaml;
        setInput(example);
        convert(example);
    ...
    return (
        ...
        <div className="flex flex-col p-2 md:p-6 space-y-6">
            ...
            <LearnButton
                ...
                variant="secondary"
                mdFilePath="content/learn/jsonyaml.md"
            />
            </div>
                ...
                {history.length > 0 && (
                    ...
                    {history.map((item, idx) => (
                        ...
                        <div className="text-xs text-muted-foreground truncate">
                            {item.options.mode === 'json-to-yaml' ? 'JSON → YAML' : 'YAML → JSON'} | {item.input?.slice(0, 40)}...
                        </div>
