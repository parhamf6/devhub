...
import jsonTomlConverter, { converterInstance, ConversionOptions, ConversionResult } from '@/lib/tools/tool/jsontoml';
...
export default function JsonTomlConverterPage() {
  ...
  // Swap direction
  const handleSwap = () => {
    const newMode = options.mode === 'json-to-toml' ? 'toml-to-json' : 'json-to-toml';
  ...
  const handleBeautify = () => {
    if (options.mode === 'json-to-toml') {
      ...
      toast.info('Beautify only for JSON input');
    }
  };
  ...
  const handleMinify = () => {
    if (options.mode === 'json-to-toml') {
      ...
      toast.info('Minify only for JSON input');
    }
  };

  // Format TOML
  const handleFormatToml = () => {
    if (options.mode === 'toml-to-json') {
      const result = converterInstance.formatToml(input, options);
      if (result.success) {
        setInput(result.output);
        toast.success('Formatted TOML');
      } else {
        toast.error(result.error || 'Format failed');
      }
    } else {
      toast.info('Format only for TOML input');
    }
  };
  ...
  const handleDownload = () => {
    if (!output) return;
    const format = options.mode === 'json-to-toml' ? 'toml' : 'json';
  ...
  // Insert example
  const handleExample = () => {
    const example = options.mode === 'json-to-toml' ? EXAMPLES.json : EXAMPLES.toml;
    setInput(example);
    convert(example);
      ...
      <div className="flex flex-col p-2 md:p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <LearnButton
            ...
            variant="secondary"
            mdFilePath="content/learn/jsontoml.md"
          />
        </div>
        ...
        {showSettings && (
          <div className="flex flex-wrap gap-4 items-center bg-muted/50 rounded-lg p-4 border border-border">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm">TOML Arrays:</span>
              <select
                value={options.tomlArrayStyle}
                onChange={e => setOptions(prev => ({ ...prev, tomlArrayStyle: e.target.value as ConversionOptions['tomlArrayStyle'] }))}
                className="rounded border border-border bg-popover px-2 py-1 text-sm"
              >
        ...
        <div className="flex flex-col gap-4">
          <div className='flex flex-wrap gap-4'>
            <Tooltip>
              ...
              <TooltipContent>Load example {options.mode === 'json-to-toml' ? 'JSON' : 'TOML'}</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
          ...
          <div className='flex gap-4 flex-wrap'>
            <Button
              variant={options.mode === 'json-to-toml' ? 'default' : 'outline'}
              onClick={() => {
                setOptions(prev => ({ ...prev, mode: 'json-to-toml' }));
                convert(input, { ...options, mode: 'json-to-toml' });
              }}
            >
              JSON → TOML
            </Button>
            <Button
              variant={options.mode === 'toml-to-json' ? 'default' : 'outline'}
              onClick={() => {
                setOptions(prev => ({ ...prev, mode: 'toml-to-json' }));
                convert(input, { ...options, mode: 'toml-to-json' });
              }}
            >
              TOML → JSON
            </Button>
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-popover rounded-t-lg">
              ...
              <div className="flex items-center gap">
                ...
                <Tooltip>
                  ...
                  <TooltipContent>Beautify JSON</TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button size="icon" variant="ghost" onClick={handleFormatToml}>
                      <Code className="h-4 w-4" />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>Format TOML</TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                  ...
                  {history.map((item, idx) => (
                    ...
                    >
                      <div className="text-xs text-muted-foreground truncate">
                        {item.options.mode === 'json-to-toml' ? 'JSON → TOML' : 'TOML → JSON'} | {item.input?.slice(0, 40)}...
                      </div>
