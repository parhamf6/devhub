...
import jsonXmlConverter, { converterInstance, ConversionOptions, ConversionResult } from '@/lib/tools/tool/jsonxml';
...
export default function JsonXmlConverterPage() {
  ...
  // Conversion logic
  const convert = async (val: string, opts = options) => {
    setIsLoading(true);
    try {
      const result = await converterInstance.convert(val, opts);
  ...
  // Swap direction
  const handleSwap = () => {
    const newMode = options.mode === 'json-to-xml' ? 'xml-to-json' : 'json-to-xml';
  ...
  const handleBeautify = () => {
    if (options.mode === 'json-to-xml') {
      const result = converterInstance.beautifyJson(input, options.indentSize);
      if (result.success) {
      ...
      const result = converterInstance.formatXml(input, options.indentSize);
      if (result.success) {
        setInput(result.output);
        toast.success('Formatted XML');
      } else {
        toast.error(result.error || 'Format failed');
      }
    }
  };
  ...
  const handleMinify = () => {
    if (options.mode === 'json-to-xml') {
      const result = converterInstance.minifyJson(input);
      if (result.success) {
        setInput(result.output);
      ...
      const result = converterInstance.minifyXml(input);
      if (result.success) {
        setInput(result.output);
        toast.success('Minified XML');
      } else {
        toast.error(result.error || 'Minify failed');
      }
    }
  ...
  const handleDownload = () => {
    if (!output) return;
    const format = options.mode === 'json-to-xml' ? 'xml' : 'json';
  ...
  // Insert example
  const handleExample = () => {
    const example = options.mode === 'json-to-xml' ? EXAMPLES.json : EXAMPLES.xml;
    setInput(example);
    convert(example);
      ...
      <div className="flex flex-col p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <LearnButton
            ...
            variant="secondary"
            mdFilePath="content/learn/jsonxml.md"
          />
        </div>
          ...
          <div className="flex flex-wrap gap-4 items-center bg-muted/50 rounded-lg p-4 border border-border">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={e => setOptions(prev => ({ ...prev, xmlDeclaration: e.target.checked }))}
                className="rounded"
              />
              XML Declaration
            </label>
            <div className="flex items-center gap-2">
              <span className="text-sm">Root Element:</span>
              <input
                type="text"
                value={options.rootElement}
                onChange={e => setOptions(prev => ({ ...prev, rootElement: e.target.value }))}
                className="w-24 rounded border border-border bg-popover px-2 py-1 text-sm"
              />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm">Attr Prefix:</span>
              <input
                type="text"
                value={options.attributeNamePrefix}
                onChange={e => setOptions(prev => ({ ...prev, attributeNamePrefix: e.target.value }))}
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={e => setOptions(prev => ({ ...prev, textNodeName: e.target.value }))}
                className="w-16 rounded border border-border bg-popover px-2 py-1 text-sm"
              />
            </div>
            <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col gap-4">
          <div className='flex flex-wrap gap-4'>
            <Tooltip>
              ...
              <TooltipContent>Load example {options.mode === 'json-to-xml' ? 'JSON' : 'XML'}</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
          ...
          <div className='flex gap-4 flex-wrap'>
            <Button
              variant={options.mode === 'json-to-xml' ? 'default' : 'outline'}
              onClick={() => {
                setOptions(prev => ({ ...prev, mode: 'json-to-xml' }));
                convert(input, { ...options, mode: 'json-to-xml' });
              }}
            >
              JSON → XML
            </Button>
            <Button
              variant={options.mode === 'xml-to-json' ? 'default' : 'outline'}
              onClick={() => {
                setOptions(prev => ({ ...prev, mode: 'xml-to-json' }));
                convert(input, { ...options, mode: 'xml-to-json' });
              }}
            >
              XML → JSON
            </Button>
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-popover rounded-t-lg">
              ...
              <div className="flex items-center gap">
                ...
                <Tooltip>
                  ...
                  <TooltipContent>Beautify {options.mode === 'json-to-xml' ? 'JSON' : 'XML'}</TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                  ...
                  <TooltipContent>Minify {options.mode === 'json-to-xml' ? 'JSON' : 'XML'}</TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                  ...
                  {history.map((item, idx) => (
                    ...
                    >
                      <div className="text-xs text-muted-foreground truncate">
                        {item.options.mode === 'json-to-xml' ? 'JSON → XML' : 'XML → JSON'} | {item.input?.slice(0, 40)}...
                      </div>
